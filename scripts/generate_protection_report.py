...
def get_resource_category(resource_type: str) -> str:
    ...
    snapshot_types = [
        ...
        'azure:sql:restorepoint', 'azure:sql:ltrbackup',
    ]
    ...
    db_types = [
        ...
        'gcp:alloydb:instance', 'gcp:bigtable:instance', 'gcp:bigquery:dataset',
    ]
    if resource_type in db_types:
        return 'Database'

    # Object storage
    object_storage = ['aws:s3:bucket', 'azure:storage:blob', 'gcp:storage:bucket']
    ...
    # Cache/In-Memory
    cache_types = ['aws:elasticache:cluster', 'azure:redis:cache', 'gcp:redis:instance']
    if resource_type in cache_types:
        return 'Cache'

    # Backup/Recovery resources
    backup_types = [
        'aws:backup:plan', 'aws:backup:vault', 'aws:backup:selection',
        'aws:backup:protected-resource', 'aws:backup:recovery-point',
        'azure:recoveryservices:vault', 'azure:backup:policy',
        'azure:backup:protecteditem', 'azure:backup:recoverypoint',
        'gcp:backupdr:plan', 'gcp:backupdr:vault',
        'gcp:backupdr:datasource', 'gcp:backupdr:backup',
    ]
...
def generate_report(inventory_path: str, output_path: str, preloaded_data: Optional[Dict[str, Any]] = None):
    ...
    get_other_primary_resources(resources)
    ...
    len([r for r in rows if 'Protected' in r['protection_status'] or 'In Backup Plan' in r['protection_status']])
    len([r for r in rows if 'Unprotected' in r['protection_status']])
    len([r for r in rows if 'In Backup Plan' in r['protection_status']])
    len([r for r in rows if r['protection_status'] == 'AMI Artifact'])
    len([r for r in rows if r['snapshot_id']])

    len(set(r['instance_id'] for r in rows if r['instance_id']))
    len(set(r['volume_id'] for r in rows if r['volume_id']))
    len(set(r['snapshot_id'] for r in rows if r['snapshot_id']))
